...
export const getProducts =
	(keywords, price, category, ratings, currentPage = 1) =>
	async (dispatch) => {
		try {
			dispatch({
			...
			let link = `/api/v1/products?keywords=${keywords}&page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}&ratings[gte]=${ratings}`;

			if (category) {
				link = `/api/v1/products?keywords=${keywords}&page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}&category=${category}&ratings[gte]=${ratings}`;
			}
...
export const checkIsBuy = (id) => async (dispatch) => {
	try {
		...
		const { data } = await axios.get(`/api/v1/${id}/isbuy`);
