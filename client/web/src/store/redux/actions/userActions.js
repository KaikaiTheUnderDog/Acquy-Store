...
export const sendOtp = (email) => async (dispatch) => {
	try {
		...
		const { data } = await axios.put('/api/v1/register/otp', email, config);
...
export const verifyEmail = (otp) => async (dispatch) => {
	try {
		...
		const { data } = await axios.put(`/api/v1/verify/${otp}`);
...
export const addShippingInfo = (shippingData) => async (dispatch) => {
	try {
		...
		const { data } = await axios.put(`/api/v1/shipping/add`, shippingData, config);
...
export const loadUser = () => async (dispatch) => {
	try {
		...
		dispatch({
			...
			payload: error.response?.data?.errMessage || error.message || 'Unknown error',
		});
	}
};
...
export const resetPassword = (password) => async (dispatch) => {
	try {
		...
		const { data } = await axios.put(`/api/v1/password/reset`, password, config);
